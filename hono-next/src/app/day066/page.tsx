...
import { AlertCircle, Calendar, CheckCircle2, Copy, DollarSign, FileJson, Image as ImageIcon, LayoutList, Loader2, Phone, Receipt, Search, ShoppingCart, Store, Trash2, Type, Upload } from 'lucide-react';
import React, { useMemo, useRef, useState } from 'react';
import { analyzeReceiptWithAI, ocrImage, parseReceiptWithRules } from './actions';
...
const OcrParserPage = () => {
  ...
  const [activeTab, setActiveTab] = useState<'table' | 'raw' | 'receipt' | 'ai'>('table');
  ...
  const handleOcrApi = async () => {
    ...
    try {
      ...
      // ルールベース解析を実行（高速）
      const summary = await parseReceiptWithRules(data.lines);
      setReceiptSummary(summary);

      // AI解析を実行
      setIsAnalyzing(true);
      try {
        const aiResult = await analyzeReceiptWithAI(data.text);
        setAiResult(aiResult);
      } catch (aiErr) {
        console.error('AI解析エラー:', aiErr);
      } finally {
          ...
          <div className="xl:col-span-8">
            {!parsedData ? (
              ...
              <div className="space-y-4">
                ...
                <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200 flex flex-wrap items-center justify-between gap-4">
                  <div className="flex bg-slate-100 p-1 rounded-lg">
                    <button
                      ...
                      className={`px-4 py-1.5 rounded-md text-sm font-medium transition-all flex items-center gap-2 ${activeTab === 'receipt' ? 'bg-white shadow-sm text-blue-600' : 'text-slate-500 hover:text-slate-700'}`}
                    >
                      <Receipt size={16} /> レシート解析
                    </button>
                    <button
                    ...
                    <button
                      ...
                      className={`px-4 py-1.5 rounded-md text-sm font-medium transition-all flex items-center gap-2 ${activeTab === 'ai' ? 'bg-white shadow-sm text-blue-600' : 'text-slate-500 hover:text-slate-700'}`}
                    >
                      {isAnalyzing ? <Loader2 size={16} className="animate-spin" /> : <Receipt size={16} />}
                      AI解析 {isAnalyzing && '中...'}
                    </button>
                  </div>
                ...
                {/* Content Area */}
                <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                  {activeTab === 'receipt' && receiptSummary && (
                    <div className="p-6">
                      {/* 基本情報 - 店舗情報 */}
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                        {receiptSummary.storeName && (
                          <div className="bg-slate-50 p-4 rounded-lg">
                            ...
                            <p className="text-slate-800 font-medium">{receiptSummary.storeName}</p>
                          </div>
                        )}
                        {receiptSummary.date && (
                          <div className="bg-slate-50 p-4 rounded-lg">
                            ...
                            <p className="text-slate-800 font-medium">{receiptSummary.date} {receiptSummary.time}</p>
                          </div>
                        )}
                        {receiptSummary.totalAmount && (
                          <div className="bg-blue-50 p-4 rounded-lg">
                            <div className="flex items-center gap-2 text-xs font-semibold text-blue-600 uppercase mb-1">
                              <DollarSign size={14} /> 合計
                            </div>
                            <p className="text-blue-800 font-bold text-xl">¥{receiptSummary.totalAmount.toLocaleString()}</p>
                          </div>
                        )}
                        {receiptSummary.paymentMethod && (
                          <div className="bg-green-50 p-4 rounded-lg">
                            <div className="flex items-center gap-2 text-xs font-semibold text-green-600 uppercase mb-1">
                              <Receipt size={14} /> 支払方法
                            </div>
                            <p className="text-green-800 font-medium">{receiptSummary.paymentMethod}</p>
                          </div>
                        )}
                      </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                        {receiptSummary.address && (
                          <div className="bg-slate-50 p-4 rounded-lg">
                            <div className="text-xs font-semibold text-slate-500 uppercase mb-1">住所</div>
                            <p className="text-slate-800 text-sm">{receiptSummary.address}</p>
                          </div>
                        )}
                        {receiptSummary.phone && (
                          <div className="bg-slate-50 p-4 rounded-lg">
                            <div className="text-xs font-semibold text-slate-500 uppercase mb-1">電話番号</div>
                            <p className="text-slate-800 text-sm">{receiptSummary.phone}</p>
                          </div>
                        )}
                        {receiptSummary.registrationNumber && (
                          <div className="bg-slate-50 p-4 rounded-lg">
                            <div className="text-xs font-semibold text-slate-500 uppercase mb-1">登録番号</div>
                            <p className="text-slate-800 text-sm">{receiptSummary.registrationNumber}</p>
                          </div>
                        )}
                      </div>
                      ...
                      {receiptSummary.items.length > 0 && (
                        <div>
                          <div className="flex items-center gap-2 text-xs font-semibold text-slate-500 uppercase mb-3">
                            <ShoppingCart size={14} /> 商品一覧 ({receiptSummary.items.length}件)
                          </div>
                          <div className="border border-slate-200 rounded-lg overflow-hidden">
                            <table className="w-full text-left">
                              <thead>
                                <tr className="bg-slate-50 border-b border-slate-200">
                                  <th className="p-3 text-xs font-semibold text-slate-500">商品名</th>
                                  <th className="p-3 text-xs font-semibold text-slate-500 text-right">金額</th>
                                </tr>
                              </thead>
                              <tbody>
                                {receiptSummary.items.map((item, idx) => (
                                  <tr key={idx} className="border-b border-slate-100 last:border-0">
                                    ...
                                    <td className="p-3 text-sm text-slate-700 text-right font-mono">¥{item.price.toLocaleString()}</td>
                                  </tr>
                                ))}
                              </tbody>
                            </table>
                  ...
                  {activeTab === 'ai' && (
                    <div className="p-6">
                      <div className="flex items-center justify-between mb-4">
                        <h4 className="text-sm font-bold text-slate-800">AI解析結果</h4>
                        {aiResult && (
                          <button
                      ...
                      {isAnalyzing ? (
                        ...
                        <pre className="bg-slate-50 p-4 rounded-lg border border-slate-200 text-sm text-slate-800 overflow-x-auto whitespace-pre-wrap font-mono">
                          {aiResult}
                        </pre>
                      ) : (
