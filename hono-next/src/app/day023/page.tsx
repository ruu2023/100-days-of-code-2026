...
import { Play, RotateCcw, CheckCircle2, AlertCircle, Database } from "lucide-react";
...
const PROBLEMS = [
  ...
  },
  {
    id: 3,
    title: "スコアが高い順に並べ替え",
    description: "全てのユーザーを score が高い順（降順）に並べ替ええて取得してください。",
...
export default function Home() {
  ...
  return (
    <main className="relative min-h-screen p-4 pb-24 md:p-8 bg-slate-900">
      <div className="max-w-4xl mx-auto space-y-6">
        <header className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-2xl font-bold tracking-tight text-white">SQL DRILL</h1>
          </div>
          <div className="flex gap-2">
             <div className="px-3 py-1 text-xs font-semibold text-blue-400 border border-blue-400/30 rounded-full bg-slate-800">
               Day 023
             </div>
          </div>
        </header>

        <section className="bg-slate-800 p-6 rounded-xl space-y-3">
          ...
          <h3 className="text-xl font-semibold text-white">{currentProblem.title}</h3>
          <p className="text-zinc-400 text-sm leading-relaxed">{currentProblem.description}</p>
        </section>

        <section className="space-y-4">
          <div className="bg-slate-800 p-2 flex flex-wrap gap-2 overflow-x-auto">
            {["SELECT", "*", "FROM", "WHERE", "ORDER BY", "DESC", "users", "name", "role", "score"].map((kw) => (
              <button
                key={kw}
                onClick={() => addKeyword(kw)}
                className="px-3 py-1.5 text-xs font-mono rounded-lg bg-slate-700 text-zinc-300 hover:bg-slate-600 whitespace-nowrap"
              >
                {kw}
              </button>
            ))}
          ...
          <div className="relative group">
            <textarea
              ...
              className="w-full h-40 p-4 bg-slate-800 border border-slate-700 rounded-2xl focus:outline-none focus:ring-2 focus:ring-blue-500/50 font-mono text-sm resize-none transition-all text-white placeholder:text-zinc-500"
            />
            {error && (
            ...
            <div className="absolute top-4 right-4 flex gap-2">
              <button onClick={handleClear} className="p-2 bg-slate-700 rounded-xl text-zinc-400 hover:bg-slate-600">
                <RotateCcw className="w-5 h-5" />
              </button>
              <button
                onClick={handleExecute}
                className="flex items-center gap-2 px-6 py-2 bg-blue-600 hover:bg-blue-500 text-white font-semibold rounded-xl transition-all active:scale-95"
              >
        ...
        {isCorrect && currentProblemIdx < PROBLEMS.length - 1 && (
          ...
          >
            <button
              ...
              className="px-8 py-3 bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-bold rounded-2xl shadow-xl"
            >
              次の問題へ
            </button>
          </motion.div>
