...
const api = {
  getCards: () =>
    apiFetch('/api/tango/cards').then((r) => r.json() as Promise<{ cards: Card[] }>),
  addCard: (front: string, back: string) =>
    apiFetch('/api/tango/cards', {
      method: 'POST',
      body: JSON.stringify({ front, back }),
    }).then((r) => r.json() as Promise<Card>),
  editCard: (id: string, front: string, back: string) =>
    apiFetch(`/api/tango/cards/${id}`, {
      method: 'PATCH',
      body: JSON.stringify({ front, back }),
    }).then((r) => r.json() as Promise<{ ok: boolean }>),
  deleteCard: (id: string) =>
    apiFetch(`/api/tango/cards/${id}`, { method: 'DELETE' }).then((r) =>
...
export default function TangoApp() {
  ...
  useEffect(() => {
    apiFetch('/api/tango/cards').then(async (r) => {
